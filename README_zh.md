...
IQ 码流采集（AD9364，采样率 40 MSps，单次捕获 262 万采样点 / 65 ms）
  ...
  第二级 — 频谱成像 + YOLOv8 推理（S2）
    ...
    bbox_score 透传至 alert_info，供 SDS 融合备用。
    【注】YOLO 辅助注入默认关闭（YOLO_ASSIST_ENABLED=False），
         待针对 5.8 GHz 瀑布图数据集重训完成后方可启用。
        │
        ▼
  第三级 — 循环频率判别器 v4.0（S3）
...
3.  **YOLO 辅助救援（可选，默认关闭）** — 完成 5.8 GHz 瀑布图数据集重训后（`config.py` 中 `YOLO_ASSIST_ENABLED = True`），S2 bbox_score 在 S3 SDS 得分 ∈ [0.85, 1.00) 时注入 +0.15 补充分，对弱信号进行救援检出。YOLO 单独永远不触发告警。
...
YOLO_ASSIST_ENABLED = False                      # 重训后改为 True 以启用 S2→SDS 注入
```
...
### YOLO 辅助功能启用条件（满足全部后将 `YOLO_ASSIST_ENABLED` 改为 `True`）

1. 完成 5.8 GHz OcuSync 瀑布图数据集采集与标注
2. 使用新数据集重训 YOLOv8，验证 mAP@0.5 ≥ 0.80
3. mock_transmitter 回放测试中 bbox_score 典型值 ≥ 0.60
4. 纯噪声环境 10 分钟测试，YOLO 虚警率 < 1%
